...
if ($mode == 'print_invoice_payment' || $mode == 'send_account_payment') {
    ...
    if (!empty($order_info['payment_info'])) {
        ...
        if (!empty($order_info['payment_info']['bank_details'])) {
            $order_info['info_customer'] .= __("addons.cryptonator.bank_details") . ': ' . $order_info['payment_info']['bank_details'];
        }
    }
    ...
    if (!empty($data_payment)) {
        ...
        if (!empty($data_payment['account_kpp'])) {
            $order_info['info_supplier'] .= __("addons.cryptonator.account_kpp") . ': ' . $data_payment['account_kpp'] . ' ';
        }
    ...
    $order_info['text_invoice_payment'] = __("addons.cryptonator.text_invoice_payment", array('[number_account]' => $order_info['order_id'], '[invoice_data]' => $invoice_date));
    ...
    if ($mode == "send_account_payment") {
        if (!empty($order_info['email'])) {
            ...
            $html = array(
                $view->displayMail('addons/cryptonator/print_invoice_payment.tpl', false, 'C')
            );
            ...
            if (@Pdf::render($html, fn_get_files_dir_path() . 'account_payment.pdf', 'save')) {
                ...
                Mailer::sendMail(array(
                    ...
                    'tpl' => 'addons/cryptonator/print_invoice_payment.tpl',
                    'is_html' => true
                ), 'A');

                fn_set_notification('N', __('notice'), __('text_email_sent'));
            } else {
                fn_set_notification('E', __('notice'), __('cryptonator.sberbank.error_text_email_not_sent'));
            }
        }
    } else {
        $view->assign('show_print_button', true);
        $view->displayMail('addons/cryptonator/print_invoice_payment.tpl', true, 'C');
    }
